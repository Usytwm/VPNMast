    ...
    def pack(self, fmt, *args):
        for i, d in zip(fmt, args):
            if i == 'B':
                ...
                if len(d) > 255:
                    raise BufferError("Error packing struct '%s' <%s>: too long string %d" % (fmt, self.data, len(d)))
                ...
                raise BufferError("Error packing struct '%s' <%s>: unknown fmt '%s'" % (fmt, self.data, i))
    def append(self, s):
        self.offset += len(s)
        self.data.extend(s)
