...
class regulation_VLAN(rule):
    ...
    def check(self, user: vpn_user, body: vpn_body) -> bool:
        ...
        if self.port == 0:
            return int(user.id_vlan) != self._id_vlan or dest_ip != self.ip
        else:
...
class regulation_User(rule):
    ...
    def check(self, user: vpn_user, body: vpn_body) -> bool:
        dest_ip = '127.0.0.1' if body.dest_ip == 'localhost' else body.dest_ip
        if self.port == 0:
            return int(user.id) != self._user_id or dest_ip != self.ip
        return int(user.id) != self._user_id or dest_ip != self.ip or self.port != body.dest_port
